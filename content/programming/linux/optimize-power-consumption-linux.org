...
This guide is focused on laptops but you can get value even for
dektop servers.
...
in the context of power management. I will give you a quick overview
of power consumption on a linux system running on the x86
architecture.
...
(using something like [[https://linux.die.net/man/1/irqbalance][irqbalance]], for example). Going back to ACPI,
hardware other than processors have "device power states" D0 to D3
where D0 means that the device is fully on, D3 that It is off. ACPI
further defines those states for many device types (network,
storage...).
...
Obviously, the more stuff you are doning with your computer, the more
energy It will consume. You should prefer light software compiled in
...
cofiguration via P-states. To list the available governors, run:
...
kernel modules for various hardware, which you do not need, and
