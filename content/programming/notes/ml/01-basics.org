...
- **Dimensionality reduction**: selection methods can be applied to
  redice the number of features while preserving the most important
  information.
...
ouputs. More technically, we are going to use the **probabilistic
model** of learning.
...
- **task**: a task represents the type of prediction being made to
  ...
  - Density estimatoin: f: x -> Delta(x)
- **data**: information about the problem to solve in the form of a
  distribution p which is tipically unknown.
